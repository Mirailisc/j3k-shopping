...
import { DropdownMenu, DropdownMenuContent, DropdownMenuItemIndicator, DropdownMenuLabel, DropdownMenuRadioGroup, DropdownMenuRadioItem, DropdownMenuSeparator, DropdownMenuTrigger } from "@radix-ui/react-dropdown-menu"
  ...
  return (
    <Card className="flex flex-col">
      ...
      <DropdownMenu>
            ...
            <DropdownMenuContent align = "end" className = "mt-[5px] px-2 py-2 border-2 rounded-sm w-30 z-1 bg-black">
             <DropdownMenuLabel >Data Type</DropdownMenuLabel>
             <DropdownMenuRadioGroup value = {dataType} onValueChange={setDataType} >
              
              {DataType.map((item) => {
                 return( <DropdownMenuRadioItem key = {item.value} value = {item.value}  className = "text-sm focus:outline-none focus:opacity-50">
